...
파일 머리의 `target triple`과 꼬리의 `attributes`, `!` 메타데이터는 환경 설정이다. 본문 해독에는 필요 없어 건너뛴다. 그리고 옛 LLVM(14 이전)은 `ptr` 대신 `i32*`로 타입까지 표기했는데, LLVM 15부터 opaque pointer로 통일됐다. 문법 세대가 다를 뿐 뜻은 같다.
