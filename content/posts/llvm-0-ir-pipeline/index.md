...
pass는 LLVM에 내장된 작은 프로그램으로, IR 전체를 한 차례 훑으며 정해진 한 가지 분석 또는 변환을 수행한다. `-O1`은 미리 정해둔 pass 목록을 순서대로 실행하는 clang 옵션이고, `opt`는 pass를 낱개로 골라 실행하는 도구다.

`-O0`부터 `-O3`까지는 포함 관계다. 레벨이 올라갈수록 목록에 pass가 추가된다.

| 옵션 | 실행되는 pass 목록 |
|---|---|
| `-O0` | 없음 (기본값) |
| `-O1` | 기본 목록. LLVM 22 기준 98항목 |
| `-O2` | `-O1` 목록에 추가해 115항목. 배포 빌드의 표준 |
| `-O3` | `-O2` 목록에 추가해 118항목. 코드 크기를 늘려서라도 속도 우선 |

목록의 실제 내용은 다음 명령이 출력한다. mem2reg(지역 변수의 메모리 왕복을 없애는 pass)도 `-O1` 목록 안에 들어 있다.
