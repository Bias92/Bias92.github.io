...
그림을 위에서부터 읽으면 이렇다. 프런트엔드인 clang이 `Test.c`를 파싱하고 타입을 검사해 IR 파일 `Test.ll`을 만든다. 미들엔드인 opt가 pass라는 변환 단위들로 IR을 고쳐 써서 최적화된 `Test.ll'`을 내놓는다. 백엔드인 llc가 그것을 CPU의 어셈블리 `Test.s`로 내리고, 어셈블러가 비트로 바꾼 조각 `Test.o`를 링커가 이어 붙여 실행 파일이 된다. 아래 절들이 이 층을 위에서부터 하나씩 따라간다.

가운데를 흐르는 IR(intermediate representation)는 소스와 기계어 사이의 중간 언어로, 프런트엔드가 만들고 pass가 고치고 백엔드가 받는 것이 전부 이 한 형식이다. 다른 컴파일러의 IR은 실행 중인 컴파일러의 메모리 안에만 존재하는 객체 구조라 파일로 적을 수 없다. LLVM IR은 문법이 공개된 텍스트 규격이라 파일로 저장하고, 손으로 고치고, 다시 컴파일러에 입력할 수 있다.
