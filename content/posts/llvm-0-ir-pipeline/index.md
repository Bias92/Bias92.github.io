...
IR(intermediate representation, 소스와 기계어 사이의 중간 언어) 하나가 프런트엔드, 미들엔드, 백엔드 세 구간을 전부 지나다닌다. 다른 컴파일러들은 IR을 메모리 안 객체로만 들고 있어서 파일로 꺼낼 수가 없다. LLVM IR은 문법이 공개된 텍스트라서, 파일로 저장해 놓고 손으로 고친 다음 컴파일러에 도로 넣는 것까지 된다.
