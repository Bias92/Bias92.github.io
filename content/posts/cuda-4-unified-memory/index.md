...
프로그램의 정답은 다음과 같다. 뒤에서 다루는 Orin 실행도 같은 출력을 냈다.
...
이 출력은 CPU write → GPU write → CPU read의 순서와 값의 가시성을 검사하는 correctness gate다. Orin에서는 뒤의 attribute 조회에서 `integrated=1`과 `concurrentManagedAccess=0`을 함께 확인했다. 결론은 **shared physical DRAM을 쓰는 integrated GPU의 limited Unified Memory**다. 다음 demand-paging 경로는 Orin trace가 아니라 full support 대조 모델이다.
...
kernel이 끝난 뒤 최신 page가 GPU memory에 있고 CPU mapping이 유효하지 않다면 CPU fault 뒤 host 쪽으로 migration이 일어난다. 포인터 값은 처음부터 끝까지 `x`지만, 그 주소에 연결된 물리 page와 접근 권한은 바뀐다.

fault와 migration은 동의어가 아니다. fault는 해결해야 할 접근 사건이고, migration은 해결 방법 중 하나다. full software-coherent model은 위처럼 page를 옮기고, remote-mapping model은 CPU-resident page를 GPU page table에 연결하며, hardware-coherent model은 공유 memory를 coherence protocol 아래 읽는다. device attribute는 이 가운데 허용되는 support class를 정한다. Orin은 limited이므로 이 세 full-support 경로가 이번 실측의 실행 모델은 아니다.
...
CUDA 문서에서 native Windows와 WSL 2는 limited model로 분류된다. RTX 4060 Ti도 native Windows에서는 compute capability 8.9와 무관하게 이 분류를 따른다. GPU 세대가 아니라 OS와 driver model이 이 경로를 결정한다.

여기까지는 CPU DRAM과 GPU memory가 분리됐다고 가정했다. CPU와 integrated GPU가 같은 physical DRAM을 쓰는 system에는 건너갈 별도 VRAM과 PCIe 전송이 없다. non-coherent system에서도 CUDA 동기화 규칙이 CPU와 GPU 사이 값의 가시성을 보장하고, hardware-coherent system은 interconnect protocol로 이를 제공한다.
...
판정은 `concurrentManagedAccess=0`에서 끝난다. `integrated=1`과 함께 보면 결론은 **shared physical DRAM + limited Unified Memory**다. 별도 VRAM으로 옮기는 PCIe migration은 없고, `cudaDeviceSynchronize()` 뒤 CPU read에서 GPU가 쓴 최신 값이 보인다. `pageableMemoryAccess=0`이므로 `malloc`을 관리 범위에 넣는 HMM 경로도 지원하지 않는다. 이 분기에서는 `usesHostPageTables=0`을 software coherence나 HMM의 증거로 해석하지 않는다. 빌드 환경과 원문 출력은 [Orin observation](/code/cuda-04/orin-jetpack-6.2.2.txt)에 남겼다.

![같은 CPU→GPU→CPU 순서를 처리하는 세 memory-management 경로](images/three-models.svg)

즉, support model과 physical topology는 서로 다른 축이다. 이번 Orin은 **shared memory + limited**다. native Windows의 RTX 4060 Ti는 **separate memory + limited**이므로 같은 limited라도 memory placement 경로는 다르다.
...
attribute는 실행 모델의 분기점이다. Orin에서는 `concurrentManagedAccess=0`에서 판정이 끝나므로 뒤의 `pageableMemoryAccessUsesHostPageTables` 값으로 HMM이나 hardware coherence를 추론하지 않는다.
...
이 표는 성능 순위가 아니라 support class에 따른 선택 기준이다. managed memory의 장점은 placement와 coherence를 runtime 쪽에 맡겨 프로그램을 단순화하는 데 있다. 이번 Orin의 결론은 limited Unified Memory에서 correctness가 성립했다는 것이며, 성능 비교는 이 글의 범위가 아니다.
