...
CUDA 문서는 이 셋을 구분해서 쓴다.

| 용어 | 무엇을 가리키나 |
|---|---|
| mapping (매핑) | 가상 페이지를 물리 프레임에 연결하는 주소 관계 |
| placement (배치) | 그 물리 프레임이 어느 메모리에 속하는가 |
| migration (이동) | 데이터를 다른 물리 메모리로 옮겨 배치를 바꾸는 일 |

배치는 관리형 할당의 각 페이지 단위로 정해진다. GPU가 CPU 메모리의 데이터를 복사하지 않고 직접 읽도록 주소 연결만 설정하면 배치는 바뀌지 않는다. 이동이 일어나야 배치가 바뀐다.
