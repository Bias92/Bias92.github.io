...
![값 41이 든 관리형 페이지가 시스템 DRAM에서 메모리 컨트롤러와 PCIe를 거쳐 discrete GPU의 VRAM으로 이동하는 경로](images/migration-placement.gif?v=1#wide)
...
여기서 최신 값은 그 주소에 가장 마지막으로 쓴 값을 말한다. 같은 주소의 사본이 DRAM, VRAM, 각 처리 장치의 캐시에 여러 개 있을 수 있고 내용이 서로 다를 수 있으므로, 어느 것이 유효한지 가리는 기준이 필요하다.
