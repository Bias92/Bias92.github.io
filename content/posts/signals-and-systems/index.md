...
이 과목은 독립변수가 시간 하나인 연속시간 신호를 다루고, 이산시간 신호는 DSP[^dsp] 과목으로 넘긴다. 아래는 교재 1.1절과 1.2절, 강의 슬라이드[^slides] 2번부터 6번에 해당한다.
...
높이는 1로 고정이고 파라미터 $\tau$[^tau]가 밑변의 폭을 정한다. $\tau = 10^8$이면 폭 $10^8$의 펄스, $\tau = 10^{-7}$이면 폭 $10^{-7}$의 펄스이고 높이는 둘 다 1이다. 불연속점은 $t = \pm\tau/2$ 두 개이므로 piecewise continuous이고, $t$가 실수 전체에서 정의되므로 연속시간 신호다.
...
불연속점은 $t = 2n \pm 1/2$에 무한 개[^train] 있으므로 continuous도 piecewise continuous도 아니다. $t$는 실수 전체에서 정의되므로 연속시간 신호다. 펄스를 유한 개로 자르면 불연속점이 유한 개가 되어 piecewise continuous가 된다.
...
[^dsp]: Digital Signal Processing, 디지털 신호처리. 강의 슬라이드 6번은 이산시간 신호를 "Covered intensively in Digital Signal Processing (DSP) courses"로 넘긴다.

[^slides]: 슬라이드 번호는 강의 슬라이드 "1. Representing Signals"(임승찬, 홍익대학교 전자전기공학부)의 우측 상단 인쇄 번호다. 타임스탬프 `[MM:SS]`는 2026-09-02 강의 녹음 전사문 기준이다.

[^detectable]: 강의 `[04:19]`. 교수는 detectable을 "검출 가능, 알아볼 수 있다"로 풀고, 강의실에서 말하는 목소리를 학생의 귀가 검출하는 상황을 예로 들었다.

[^variables]: 강의 `[09:11]`, `[10:45]`. 목소리의 진폭이 시간에 따라 바뀌는 것을 $A(t)$로, 화면의 픽셀값이 두 좌표에 따라 정해지는 것을 $p(x, y)$로 모델링했다. 전기장은 전자기학에서 공간 좌표 셋과 시간까지 독립변수가 넷이 된다.

[^axis]: 강의 `[26:38]`, `[37:56]`. 연속시간과 이산시간을 가를 때는 함수값이 아니라 독립변수 축을 본다. 함수값의 불연속점이 몇 개 있어도 $t$가 연속 구간 전체에서 정의되면 연속시간 신호다.

[^sampling]: 슬라이드 6번은 $T_s$를 "a fixed positive real number"로만 적는다. DSP에서 쓰는 이름은 sampling period(샘플링 주기)이고, 역수 $f_s = 1/T_s$가 sampling frequency(샘플링 주파수)다. 강의 `[39:36]`에서 $T_s = 2$, $k = 0, \pm 1, \pm 2$를 넣어 $t = 0, \pm 2, \pm 4$를 만드는 예를 들었다.

[^piecewise]: 슬라이드 3번 원문은 "piecewise continuous if it has only finite discontinuities"다. 강의 `[36:06]`에서 유한 개의 펄스로 이루어진 트레인은 piecewise continuous이고, 무한히 이어지는 트레인은 불연속점이 무한 개라서 piecewise continuous가 아니라고 구분했다.

[^average]: 슬라이드 3번, 강의 `[24:31]`. 일반 수학에서는 undefined로 두는 값을 교재가 좌우극한의 평균으로 약속한 것이다. 이 약속 하나로 교재의 여러 결과를 설명할 수 있어서 이렇게 정의한다고 교수가 설명했다.

[^tau]: 강의 `[31:02]`. $\tau$를 $10^8$으로 잡으면 밑변이 $\pm 5 \times 10^7$까지 벌어지고, $10^{-7}$로 잡으면 아주 좁은 펄스가 되지만 높이는 두 경우 모두 1이다.

[^train]: 슬라이드 5번은 펄스 트레인을 "Continuous at all $t$ except at $t = 0, \pm 1, \pm 2, \cdots$"로 적는다. 강의 `[36:06]`에서 불연속점이 무한 개인데도 연속시간 신호인 이유를 독립변수 축으로 설명했다.
