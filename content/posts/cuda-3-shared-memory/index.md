...
series: ["CUDA C"]
math: true
summary: "RTX 4060 Ti에서 tiled GEMM, transpose, reduction을 구현하고 Nsight Compute 카운터로 확인한다. coalescing의 sector 규칙, bank conflict의 padding·swizzle 해법, occupancy 한도, reduction 4단계 개선과 CUB 비교. 전체 소스와 재현 명령 포함."
---
...
느린 요인이 둘이다. 첫째는 divergence다. warp 안에서 branch 결과가 갈리면 scheduler는 각 경로를 순서대로 발행하고 active mask로 해당 lane만 활성화한다. s=1에서 active mask가 `0x55555555`, s=2에서 `0x11111111`로, 일하는 lane이 16, 8, 4, ...개로 줄어도 warp 명령은 계속 발행되므로 그 비용을 매 단계 낸다. 둘째는 `%` 연산이다. 제수 `2 * s`가 loop마다 바뀌어 컴파일러가 상수 bit mask로 접지 못하고, sm_89 SASS에 `IABS → I2F → MUFU.RCP → F2I → IMAD.HI`로 이어지는 나머지 계산이 남는다(`cuobjdump --dump-sass`로 확인). 버전 1의 SASS에는 이 명령 묶음이 없다.
...
일하는 thread를 앞쪽으로 몰면 s=128에서는 앞 warp 4개가 통째로 일하고 s=64에서 2개, s=32에서 1개다. 노는 warp는 branch를 통째로 건너뛰므로 divergence가 아니다. divergence는 같은 warp 안에서 선택이 갈릴 때만 생긴다. `buf[tid]`와 `buf[tid + s]`는 연속 주소라 bank conflict도 없고 `%`도 사라진다. barrier 수는 버전 0과 같은데 1.63배 빠르다. 버전 0의 비용은 동기화가 아니라 인덱싱이었다.

![v0은 활성 lane이 warp 안에 흩어지고, v1은 warp 단위로 앞에 몰린다](images/reduction-lanes.svg)

버전 2, warp shuffle. s=16부터는 첫 warp 안에서 절반만 켜져 divergence가 다시 시작된다. 그 경계부터는 shared memory와 `__syncthreads()` 없이 레지스터에서 접는다.
...
| --- | --- | --- | --- | --- |
| v0 발산 인덱싱 | 0.625 ms | 107.3 | 37% | 93.6% |
...
카운터 관련 주의 하나. divergence 개선(v0 → v1)은 커널 전체 평균 지표에 나타나지 않는다. `smsp__average_thread_inst_executed_per_inst_executed`는 v0 = 31.93, v1 = 31.81로 거의 같다. 실행의 대부분이 global load 구간이고 거기서는 32개 lane이 전부 활성이라, 트리 구간의 발산이 평균에 희석된다. 차이는 시간과 SASS에서 확인해야 한다. 집계 지표는 구간 문제를 숨길 수 있다.
...
- [CUDA C++ Best Practices Guide](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/): coalescing, shared memory, bank conflict, occupancy의 기준 문서
- [CUDA C++ Programming Guide](https://docs.nvidia.com/cuda/cuda-c-programming-guide/): divergence, 동기화, atomics, warp intrinsic의 정확한 의미
